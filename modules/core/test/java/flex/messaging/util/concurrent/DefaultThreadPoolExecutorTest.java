...
public class DefaultThreadPoolExecutorTest extends TestCase
{
    ...
    public void testFailedExecutionHandling()
    {
        ...
        try
        {
            boolean success = executorPauseSignal.await(5, TimeUnit.MINUTES);
            if (!success)
            {
                fail("Test timed out waiting for execution to complete.");
            }
    ...
    static class Task implements Runnable
    {
        ...
        public void run()
        {
            if (taskPauseSignal != null)
            {
                try
                {
                    boolean success = taskPauseSignal.await(5, TimeUnit.MINUTES);
                    if (!success)
                    {
                        test.fail("Test timed out waiting for execution to complete.");
                    }
                    else
                    {
                ...
                {
                    test.fail("Test was interrupted: " + e);
                }
            }
